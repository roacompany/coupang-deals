import { ArrowRight, Baby, Flame, Heart, Sparkles } from "lucide-react"
...
export default function HomePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      {/* Hero */}
      <section className="bg-gradient-to-br from-pink-400 via-pink-500 to-rose-600 rounded-2xl p-6 md:p-10 mb-6 text-white">
        <div className="flex items-center gap-2 mb-2">
          <Baby className="h-5 w-5 text-pink-200" />
          <span className="text-sm font-medium text-pink-100">매일 업데이트</span>
        </div>
        <h1 className="text-2xl md:text-4xl font-bold mb-2">
          우리 아이를 위한 추천 딜
        </h1>
        <p className="text-pink-100 text-sm md:text-base mb-4">
          {SITE_NAME}이 엄선한 육아용품을 쿠팡 최저가로 만나보세요
        </p>
        <div className="flex items-center gap-3 text-xs">
          <span className="bg-white/20 px-2.5 py-1 rounded-full flex items-center gap-1">
            <Heart className="h-3 w-3" /> 안전인증 상품
          </span>
          <span className="bg-white/20 px-2.5 py-1 rounded-full flex items-center gap-1">
            <Sparkles className="h-3 w-3" /> 로켓배송 무료배송
          </span>
        </div>
      </section>
